...
async def lifespan(app: FastAPI):
    ...
    # Preload Faust Sound Skill to eliminate cold start latency
    try:
        skills_dir = Path(__file__).resolve().parent.parent / ".claude" / "skills"
        if str(skills_dir) not in sys.path:
            sys.path.insert(0, str(skills_dir))
        from sound import preload
        preload()
        print("[VOICE] Faust Sound Skill preloaded successfully.")
    except Exception as e:
        print(f"[WARNING] Failed to preload Faust Sound Skill: {e}")
...
try:
    ...
    if str(SKILLS_DIR) not in sys.path:
        sys.path.insert(0, str(SKILLS_DIR))
    from sound.scripts.api import include_voice_router
